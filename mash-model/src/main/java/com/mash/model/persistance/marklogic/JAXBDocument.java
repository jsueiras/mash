package com.mash.model.persistance.marklogic;
/*
...
public class JAXBDocument {
	...
	public static void runCrime(Util.ExampleProperties props) throws JAXBException {
		// create the client
		DatabaseClient client = DatabaseClientFactory.newClient(
				props.host, props.port, props.writerUser, props.writerPassword,
				props.authType);

		JAXBContext context = JAXBContext.newInstance(Crime.class);
		...
		// a specific class and call the get(class) method instead of
		// the get() method.
		JAXBHandle<Crime> handle = new JAXBHandle<Crime>(context);
		...
		// create an identifier for the document
		String docId = "/example/"+crime.getId()+".xml";
